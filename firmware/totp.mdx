...
TOTP stands for **Time-based One-Time Password** — the same system used by apps like Google Authenticator or Authy.\
...
Because the calculation is deterministic and offline, you get the same code as any authenticator app would — but generated entirely within your ZeroKeyUSB.

---

## Adding a 2FA secret

When you add a new account to ZeroKeyUSB, you can optionally include a **TOTP secret** (a short string provided by the service you’re securing).\
...
Tapping the center point opens the TOTP view, where you’ll see:
...
> 🧠 You can use this code anywhere you’d use Google Authenticator — it’s fully standard and compatible.

---

## Time synchronization

Since ZeroKeyUSB works offline, it doesn’t have an internal clock.\
Instead, it synchronizes **once** with your computer or phone through a simple USB command.

When the device needs to generate a code but time hasn’t been set, it shows:
...
> ⏳ You only need to set the time after a factory reset or if it drifts by several minutes.
...
| **Offline operation** | No Wi-Fi, Bluetooth, or sync — codes exist only inside your device.                   |
...
Even with physical access to the memory chip, it’s mathematically impossible to reconstruct your codes without your personal key.
...
| ---------------- | ----------------- | -------------------------- |
| Internet access  | Required for sync | ❌ Offline only             |
| Cloud backup     | External          | ❌ None — encrypted locally |
| Malware exposure | Possible          | ❌ Isolated firmware        |
| Screen privacy   | Shared device     | ✅ Dedicated hardware       |
| Power source     | Battery dependent | ✅ USB powered              |
| Auditability     | Closed source     | ✅ Fully open source        |
...
- Add 2FA only for accounts you control — avoid secrets from shared devices.
- Back up credentials before performing a factory reset.
- When prompted to “scan QR code” by a website, choose “Enter key manually” and copy the secret for ZeroKeyUSB import.

---

<Note>
  ZeroKeyUSB’s TOTP engine follows the [RFC 6238 standard](https://datatracker.ietf.org/doc/html/rfc6238),\
